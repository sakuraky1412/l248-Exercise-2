...
def extract_feature():
    ...
    fv_histogram_features = []
    fv_haralick_features = []
    fv_hu_moments_features = []
    rgb_colors_features = []
    gray_featuress = []
    edge_featuress = []
    gabor_featuress = []
        ...
        if not label.startswith('.'):
            ...
            for img_id, image_name in enumerate(image_names):
                if not image_name.startswith('.'):
                    ...
                    fv_histogram_features.append(fv_histogram)
                    fv_haralick_features.append(fv_haralick)
                    fv_hu_moments_features.append(fv_hu_moments)
                    rgb_colors_features.append(rgb_colors)
                    gray_featuress.append(gray_features)
                    edge_featuress.append(edge_features)
                    gabor_featuress.append(gabor_features)
                    orb_featuress.append(orb_features)
                    kaze_featuress.append(kaze_features)
    ...
    rescaled_features = scaler.fit_transform(all_features)
    ...
    h5f_data.create_dataset('dataset_9', data=np.array(fv_hu_moments_features))
    h5f_data.create_dataset('dataset_10', data=np.array(fv_haralick_features))
    h5f_data.create_dataset('dataset_11', data=np.array(fv_histogram_features))
    h5f_data.create_dataset('dataset_12', data=np.array(rgb_colors_features))
    h5f_data.create_dataset('dataset_13', data=np.array(gray_featuress))
    h5f_data.create_dataset('dataset_14', data=np.array(edge_featuress))
    h5f_data.create_dataset('dataset_15', data=np.array(gabor_featuress))
    h5f_data.create_dataset('dataset_16', data=np.array(orb_featuress))
    h5f_data.create_dataset('dataset_17', data=np.array(kaze_featuress))
...
def classify():
    ...
    fv_hu_moments_string = h5f_data['dataset_9']
    fv_haralick_string = h5f_data['dataset_10']
    fv_histogram_string = h5f_data['dataset_11']
    rgb_colors_string = h5f_data['dataset_12']
    gray_featuress_string = h5f_data['dataset_13']
    edge_featuress_string = h5f_data['dataset_14']
    gabor_featuress_string = h5f_data['dataset_15']
    orb_featuress_string = h5f_data['dataset_16']
    kaze_featuress_string = h5f_data['dataset_17']

    global_labels_string = h5f_label['dataset_1']
    trainLabelsGlobal = np.array(global_labels_string)

    global_base_features = np.array(base_features_string)
    global_color_features = np.array(color_features_string)
    global_edge_texture_features = np.array(edge_texture_features_string)
    global_invariant_features = np.array(invariant_features_string)
    global_all_features = np.array(all_features_string)
    global_wo_color_features = np.array(wo_color_features_string)
    global_wo_edge_textures = np.array(wo_edge_texture_features_string)
    global_wo_invariant_features = np.array(wo_invariant_features_string)
    global_fv_hu_moments = np.array(fv_hu_moments_string)
    global_fv_haralick = np.array(fv_haralick_string)
    global_fv_histogram = np.array(fv_histogram_string)
    global_rgb_colors = np.array(rgb_colors_string)
    global_gray_featuress = np.array(gray_featuress_string)
    global_edge_featuress = np.array(edge_featuress_string)
    global_gabor_featuress = np.array(gabor_featuress_string)
    global_orb_featuress = np.array(orb_featuress_string)
    global_kaze_featuress = np.array(kaze_featuress_string)
    ...
    fv_hu_moments_clf = SVC(kernel='linear', C=1.0, random_state=seed)
    fv_hu_moments_clf.fit(global_fv_hu_moments, trainLabelsGlobal)
    fv_haralick_clf = SVC(kernel='linear', C=1.0, random_state=seed)
    fv_haralick_clf.fit(global_fv_haralick, trainLabelsGlobal)
    fv_histogram_clf = SVC(kernel='linear', C=1.0, random_state=seed)
    fv_histogram_clf.fit(global_fv_histogram, trainLabelsGlobal)
    rgb_colors_clf = SVC(kernel='linear', C=1.0, random_state=seed)
    rgb_colors_clf.fit(global_rgb_colors, trainLabelsGlobal)
    gray_featuress_clf = SVC(kernel='linear', C=1.0, random_state=seed)
    gray_featuress_clf.fit(global_gray_featuress, trainLabelsGlobal)
    edge_featuress_clf = SVC(kernel='linear', C=1.0, random_state=seed)
    edge_featuress_clf.fit(global_edge_featuress, trainLabelsGlobal)
    gabor_featuress_clf = SVC(kernel='linear', C=1.0, random_state=seed)
    gabor_featuress_clf.fit(global_gabor_featuress, trainLabelsGlobal)
    orb_featuress_clf = SVC(kernel='linear', C=1.0, random_state=seed)
    orb_featuress_clf.fit(global_orb_featuress, trainLabelsGlobal)
    kaze_featuress_clf = SVC(kernel='linear', C=1.0, random_state=seed)
    kaze_featuress_clf.fit(global_kaze_featuress, trainLabelsGlobal)
    ...
    cnn_model = tflearn.DNN(convnet, tensorboard_dir='log')

    # Splitting the testing data and training data
    '''Setting up the features and lables'''
    # X-Features & Y-Labels
    X, test_x, Y, test_y = train_test_split(global_gray_featuress, trainLabelsGlobal, test_size=test_size,
                                            random_state=seed)
    ...
    # epoch = 5 taken
    cnn_model.fit({'input': X}, {'targets': Y}, n_epoch=5,
        ...
        if not label.startswith('.'):
            ...
            for img_id, image_name in enumerate(image_names):
                if not image_name.startswith('.'):
                    ...
                    # scale features in the range (0-1)
                    scaler = MinMaxScaler(feature_range=(0, 1))
                    rescaled_feature = scaler.fit_transform(all_feature.reshape(-1, 1))
                    ...
                    fv_histogram_prediction = fv_histogram_clf.predict(fv_histogram.reshape(1, -1))[0]
                    fv_haralick_prediction = fv_haralick_clf.predict(fv_haralick.reshape(1, -1))[0]
                    fv_hu_moments_prediction = fv_hu_moments_clf.predict(fv_hu_moments.reshape(1, -1))[0]
                    rgb_colors_prediction = rgb_colors_clf.predict(rgb_colors.reshape(1, -1))[0]
                    gray_featuress_prediction = gray_featuress_clf.predict(gray_features.reshape(1, -1))[0]
                    edge_featuress_prediction = edge_featuress_clf.predict(edge_features.reshape(1, -1))[0]
                    gabor_featuress_prediction = gabor_featuress_clf.predict(gabor_features.reshape(1, -1))[0]
                    orb_featuress_prediction = orb_featuress_clf.predict(orb_features.reshape(1, -1))[0]
                    kaze_featuress_prediction = kaze_featuress_clf.predict(kaze_features.reshape(1, -1))[0]

                    cnn_model_prediction = cnn_model.predict([data])[0]
                    ...
                    if label[4:] == train_labels[all_prediction]:
                        all_correct_count += 1
                    if label == train_labels[all_prediction]:
                        all_correct_count += 1
                    ...
                    total_count += 1
                    X_test.append(all_feature)
                    y_test.append(label)
                    all_pred.append(train_labels[all_prediction])
                    fv_hu_moments_pred.append(train_labels[fv_hu_moments_prediction])
                    fv_haralick_pred.append(train_labels[fv_haralick_prediction])
                    fv_histogram_pred.append(train_labels[fv_histogram_prediction])
                    rgb_colors_pred.append(train_labels[rgb_colors_prediction])
                    gray_featuress_pred.append(train_labels[gray_featuress_prediction])
                    edge_featuress_pred.append(train_labels[edge_featuress_prediction])
                    gabor_featuress_pred.append(train_labels[gabor_featuress_prediction])
                    orb_featuress_pred.append(train_labels[orb_featuress_prediction])
                    kaze_featuress_pred.append(train_labels[kaze_featuress_prediction])
                    cnn_model_pred.append(cnn_model_prediction)
    ...
    h5f_data.create_dataset('dataset_1', data=np.array(X_test))
    h5f_data.close()

    print(classification_report(y_test, all_pred, target_names=train_labels))
    print(classification_report(y_test, fv_haralick_pred, target_names=train_labels))
    print(classification_report(y_test, fv_histogram_pred, target_names=train_labels))
    print(classification_report(y_test, fv_hu_moments_pred, target_names=train_labels))
    print(classification_report(y_test, rgb_colors_pred, target_names=train_labels))
    print(classification_report(y_test, gray_featuress_pred, target_names=train_labels))
    print(classification_report(y_test, edge_featuress_pred, target_names=train_labels))
    print(classification_report(y_test, gabor_featuress_pred, target_names=train_labels))
    print(classification_report(y_test, orb_featuress_pred, target_names=train_labels))
    print(classification_report(y_test, kaze_featuress_pred, target_names=train_labels))
    print(classification_report(y_test, cnn_model_pred, target_names=train_labels))
    # import some data to play with
    h5f_data = h5py.File('Output/test_data.h5', 'r')
    X_test_string = h5f_data['dataset_1']
    X_test = np.array(X_test_string)
    h5f_data.close()
    ...
    for title, normalize in titles_options:
        disp = plot_confusion_matrix(all_clf, X_test, y_test,
